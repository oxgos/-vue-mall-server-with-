<template>
    <div class="address">
                    ...
                    <div class="addr-list">
                        <ul>
                            <li v-for="(addr, index) in limitAddr" :class="{'check': checkIndex === index}" @click="checkIndex = index">
                                ...
                                <div class="addr-opration addr-del" @click="mdShow = true; removeAddrId = addr.addressId">
                                    <a href="javascript: void(0);" class="addr-del-btn">
                                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                                    </a>
                                </div>
                                <div class="addr-opration addr-set-default">
                                    <a href="javascript: void(0);" class="addr-set-default-btn" v-if="!addr.isDefault" @click="setDefault(addr.addressId)"><i>Set default</i></a>
                                </div>
                                <div class="addr-opration addr-default" v-if="addr.isDefault">Default address</div>
                            </li>
                            <li class="addr-new">
        ...
        <modal v-if="mdShow" :modalFlag="mdShow" @changeFlag="closeModal">
            <p slot="message">是否删除地址？</p>
            <div slot="btnGroup">
                <a class="btn btn--m" href="javascript: void(0);" @click="removeAddr">确认</a>
                <a class="btn btn--m btn--red" href="javascript: void(0);" @click="mdShow = false">取消</a>
            </div>
        </modal>
    </div>
</template>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            ...
            // 设置默认地址
            setDefault (id) {
                this.$ajax.post('/users/address/setDefault', {
                    productId: id
                }).then((res) => {
            ...
            // 删除地址
            removeAddr () {
                this.$ajax.delete('/users/address/remove', {
                    params: {
                        addressId: this.removeAddrId
                    }
                }).then((res) => {
                    this.mdShow = false
                    if (res.data.status === '0') {
